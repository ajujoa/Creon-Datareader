...
def save_results(results, folder_path):
    ...
    result_file = result_dir / f"analysis_result_{folder_name}_{timestamp}.json"
...
def process_single_file(file, selected_folder):
    """
    ...
    try:
        ...
        if valid_dates:
            ...
            if not backtest_df.empty:
                ...
                # 백테스팅 결과 저장
                backtest_file = save_backtest_results(backtest_df, selected_folder, code)
...
def process_chunk(chunk_files, selected_folder):
    """
    파일 청크를 처리하는 함수
    """
    chunk_results = []
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_file = {
            executor.submit(process_single_file, file, selected_folder): file 
...
def save_backtest_results(results_df, folder_path, stock_code):
    """
    백테스팅 결과를 엑셀 파일로 저장
    ...
    excel_file = backtest_dir / f"backtest_{stock_code}_{timestamp}.xlsx"
    ...
    stats_df = pd.DataFrame({
        ...
        '값': [total_trades, win_trades, lose_trades, round(win_rate, 2), round(avg_return, 2)]
    })
    ...
    print(f"총 {total_trades}건 거래, 승률 {win_rate:.1f}%, 평균 수익률 {avg_return:.2f}%")
    
    return excel_file

def combine_all_backtest_results(folder_path):
    """
    ...
    # 모든 백테스트 엑셀 파일 찾기
    excel_files = list(backtest_dir.glob('backtest_A*.xlsx'))
    if not excel_files:
        print("백테스트 결과 파일이 없습니다.")
        return None
    
    print(f"총 {len(excel_files)}개의 백테스트 파일을 합치는 중...")
    ...
    for excel_file in excel_files:
        try:
            ...
            if not stats_df.empty:
                # 파일명에서 종목코드 추출
                stock_code = excel_file.name.split('_')[1]
    ...
    total_stats = pd.DataFrame({
        '항목': ['총 거래수', '수익 거래', '손실 거래', '승률(%)', '평균 수익률(%)', '최대 수익률(%)', '최소 수익률(%)'],
        '값': [total_trades, win_trades, lose_trades, round(win_rate, 2), 
               round(avg_return, 2), round(max_return, 2), round(min_return, 2)]
    })
    ...
    # 결과 파일 저장
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    combined_file = backtest_dir / f"combined_backtest_{timestamp}.xlsx"
...
def save_backtest_results_integrated(results_df, folder_path, stock_code, condition_set):
    ...
    # 결과를 저장할 backtest_integrated 폴더 생성
...
def combine_integrated_results(all_results, folder_path):
    ...
    backtest_dir = folder_path / 'backtest_integrated'
    if not backtest_dir.exists():
        print("backtest_integrated 폴더가 존재하지 않습니다.")
        return None
